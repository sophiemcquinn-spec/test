...
with col2:
    ...
    st.metric("Number of Transactions", f"{count:,}")
#with col3:
...
if len(filtered_df) > 0:
    ...
    # Aggregate inflow by entity type
    inflow = contributions_df.groupby('ENTITY_LABEL')['AMOUNT'].sum().reset_index()
    inflow = inflow[inflow['value'] > 0]

    outflow = expenditures_df.groupby('CATEGORY_DESC')['AMOUNT'].sum().reset_index()
    outflow = outflow[outflow['value'] > 0]

    if len(inflow) > 0:
        all_nodes = list(inflow['contribution_type']) + [selected_candidate] + list(outflow['CATEGORY_DESC'])
        node_dict = {node: i for i, node in enumerate(all_nodes)}

        sources, targets, values = [], [], []

        for _, row in inflow.iterrows():
            sources.append(node_dict[row['contribution_type']])
            targets.append(node_dict[selected_candidate])
            values.append(row['AMOUNT'])

        for _, row in outflow.iterrows():
            sources.append(node_dict[selected_candidate])
            targets.append(node_dict[row['CATEGORY_DESC']])
            values.append(row['AMOUNT'])

        fig = go.Figure(data=[go.Sankey(
            node=dict(label=all_nodes, pad=20, thickness=20),
            link=dict(source=sources, target=targets, value=values)
        )])

        fig.update_layout(
            title=f"Campaign Finance Flow for {selected_candidate} ({selected_year})",
            height=600
        )
